...
Production deployment uses Docker Compose with PostgreSQL:

```bash
# 1. Set environment variables in .env
cp .env.example .env
# Edit .env and set: DOMAIN, POSTGRES_PASSWORD, UCAN_SECRET, ALLOWED_ORIGINS

# 2. Generate master encryption key
bun run key:generate

# 3. Create Docker network
docker network create keycast

# 4. Build and run (PostgreSQL + Unified Keycast service)
...
- `keycast`: Unified binary (API + Signer + Web) in single container

**Ports:**
- 3000: API endpoints
- 5173: Web frontend

Reverse proxy: Caddy labels configured for automatic SSL cert generation.
